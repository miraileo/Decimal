...
START_TEST(add_0) {
  s21_decimal val1 = {{15, 0, 0, ~(MAX_INT / 2)}};
...
START_TEST(add_1) {
  ...
  s21_decimal val2 = {{15, 0, 0, ~(MAX_INT / 2)}};
  s21_decimal res = {{0}};
  ck_assert_int_eq(0, s21_add(val1, val2, &res));
}
END_TEST
...
START_TEST(add_test_28) {
  ...
  ck_assert_float_eq_tol(float_res, got_float, 1.e-3);
}
...
START_TEST(add_test_30) {
  s21_decimal src1 = {{MAX_INT, MAX_INT, MAX_INT, ~(INT_MAX)}};
  s21_decimal src2 = {0};
  float a = -0.01448;
  s21_from_float_to_decimal(a, &src2);
  s21_decimal res_dec = {0};
...
START_TEST(s21_test_decimal_add_0) {
  ...
  ck_assert_int_eq(
      res.bits[3],
      etalon.bits[3]);  // Возвращаемое значение: 0 - FALSE 1 - TRUE
...
START_TEST(s21_test_decimal_add_11) {
  s21_decimal c = {{0, MAX_INT - 1, 0, 0}};
  s21_decimal d = {{0, 1, 0, 0}};
  s21_decimal etalon = {{0, MAX_INT, 0, 0}};
...
START_TEST(s21_test_decimal_add_12) {
  s21_decimal c = {{0, 0, MAX_INT - 1, 0}};
  s21_decimal d = {{0, 0, 1, 0}};
  s21_decimal etalon = {{0, 0, MAX_INT, 0}};
...
START_TEST(s21_test_decimal_add_27) {
  s21_decimal c = {{MAX_INT, MAX_INT, 0, 0}};
  s21_decimal d = {{0b00000000000000000000000000000001, 0, 0, 0}};
...
Suite* s21_add_test(void) {
  Suite* s = suite_create("\033[42m-=S21_ADD=-\033[0m");
