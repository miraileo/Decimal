...
START_TEST(mul_12) {
  ...
  ck_assert_int_eq(result.bits[0], 35);
...
//   ck_assert_int_eq(result.bits[1], 0xFFFFFFFE);
...
START_TEST(mul_15) {
  ...
  s21_decimal result = {{0, 0, 0, 0}};
  int return_value = s21_mul(value_1, value_2, &result);
  ck_assert_int_eq(result.bits[0], 0xcedabe40);
  ck_assert_int_eq(result.bits[1], 0x99c0c5d);
  ck_assert_int_eq(result.bits[2], 0x13a3a);
  ck_assert_int_eq(result.bits[3], 0x80050000);
  ck_assert_int_eq(return_value, 0);
}
END_TEST
...
START_TEST(mul_test_1) {
  ...
  s21_decimal check = {{100, 0, 0, 0}};
  s21_mul(a, b, &res_dec);
  ck_assert_int_eq(res_dec.bits[0], check.bits[0]);
  ck_assert_int_eq(res_dec.bits[1], check.bits[1]);
  ck_assert_int_eq(res_dec.bits[2], check.bits[2]);
  ck_assert_int_eq(res_dec.bits[3], check.bits[3]);
}
END_TEST
...
Suite *s21_mul_test(void) {
  Suite *s = suite_create("\033[42m-=S21_MUL=-\033[0m");
